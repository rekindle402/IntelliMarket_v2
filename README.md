...
### QueryDSL 공통화
동적 검색 조건이 필요한 도메인마다 `SellerQueryRepository`처럼 별도 QueryDSL 레포지토리를 만들고, `QuerydslSearchRepository` 인터페이스의 `default` 메서드로 페이징 로직을 공유합니다.
...
**[문제]**
QueryDSL 5.x에서 `fetchResults()`가 deprecated됩니다. 또한 도메인마다 반복되는 페이징 로직을 추상 클래스로 공통화하려 했으나, JPA 레포지토리(`JpaRepository`)와의 다중 상속 불가 문제가 있었습니다.
